...
//        }
        ...
        val hover_tool = new HoverTool().plot(plot)
                         //.renderers(holidays_renderer :: Nil)
                         .tooltips(Tooltip("Holiday" -> "@month_holidays"))
        plot.tools := hover_tool :: Nil

        plot.renderers := xaxis :: days_renderer :: text_renderer :: Nil
                       //:: holidays_renderer 
    ...
    val html = document.save("calendars.html")
    print(s"Wrote ${html.file}. Open ${html.url} in a web browser.")
    html.view
}