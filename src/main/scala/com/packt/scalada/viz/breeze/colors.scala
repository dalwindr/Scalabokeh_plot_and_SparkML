...
object colors extends App {
    ...
    object source extends ColumnDataSource {
        ...
        val colorsX = column(css3_colors.map(_._2))
        val groups = column(css3_colors.map(_._3))
    }

    import source.{names,colorsX,groups}

    val xdr = new FactorRange().factors(groups.value.distinct)
    val ydr = new FactorRange().factors(names.value.reverse)

    val plot = new Plot().title("CSS3 Color Names").x_range(xdr).y_range(ydr).width(1400).height(4000)

    // TODO: categorical dimensions; using Column would cause type error
    val rect_glyph = new Rect().x('groups).y('names).width(1).height(1).fill_color(colorsX).line_color()
    ...
    val html = document.save("colors.html")
